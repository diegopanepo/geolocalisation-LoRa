 ...
 *  along with the program.  If not, see <http://www.gnu.org/licenses/>.te
 *
 *****************************************************************************
...
const uint32_t DEFAULT_CHANNEL=CH_17_868;//CH_04_868
#else
const uint32_t DEFAULT_CHANNEL=CH_17_868;//CH_10_868
#endif
...
void loop(void)
{
  ...
  PRINTLN;
    ///////
    int static i=1;
    String resultat= "{\"source\":9,\"packet_number\":";
    String indice =String(i);
    uint8_t power;
    resultat += indice;
    resultat += "}";
    
    Serial.println(resultat.c_str());
   // resultat+= ",\"pressure\":995.50,\"temperature\":28.70,\"humidity\":53.50,\"accelerometer\":{\"x\":143.00,\"y\":23.00,\"z\":-99.99},\"latitude\":37.07,\"longitude\":7.05,\"altitudeGps\":224.06,\"battery_voltage\":3.82}";
    uint8_t buff[20]={113,0,5,0,5,0,5,136,0,0,0,5,0,0,0,5,2,0,5,0};
      r_size=sprintf((char*)message, resultat.c_str());//mettre donnees ici
    
     // PRINT_CSTSTR("%s",msg.c_str()); 
      // Serial.println( resultat.c_str()); 
      PRINTLN;
            
      e = sx1272.sendPacketTimeoutACK(DEFAULT_DEST_ADDR, buff, r_size);
      // this is the no-ack version
      //e = sx1272.sendPacketTimeout(DEFAULT_DEST_ADDR, message, r_size);
            
         Serial.print("avant while e=");
          Serial.print(e);
   //  _packetNumber++;
    // sx1272._packetNumber++;
        //  if ( sx1272._packetNumber==7) sx1272._packetNumber++;
         // if ( sx1272._packetNumber==13) sx1272._packetNumber++;
         // if ( sx1272._packetNumber>15) sx1272._packetNumber++;
      if( e==3){
      Serial.print("attendre ACK");
      delay(30000);   
        while(e==3){
          power= sx1272.getPower();
            Serial.print("power=");
            Serial.print(power);
            PRINT_CSTSTR("%s","No Pong from gw!");
            Serial.print("REENVOYER MESSAGE");
          
             e = sx1272.sendPacketTimeoutACK(DEFAULT_DEST_ADDR, buff, r_size);
             Serial.println(resultat.c_str());
            Serial.print("dans le while e=");
            Serial.print(e);
            delay(10000);
        }
      }
