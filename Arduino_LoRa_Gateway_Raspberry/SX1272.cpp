...
/*
 Function: Configures the module to receive information.
 Returns: Integer that determines if there has been any error
   state = 5  --> The packet header (packet type) has not been recognized //Receive error 5
...
boolean	SX1272::availableData(uint16_t wait)
{
    ...
    { // Checking destination
       ...
       /* Serial.println(F("## JFL Destination: "));
        Serial.println(_destination);
        Serial.println(F("## JFL nodeAddress: "));
        Serial.println(_nodeAddress);
        Serial.println(F("## JFL ### "));*/
        //JFL
        _destination = _nodeAddress;
    ...
    if( p_received == true )
    {
        ...
        if (!_rawFormat) {
            ...
            if ( ((packet_received.type & PKT_TYPE_MASK) != PKT_TYPE_DATA) && ((packet_received.type & PKT_TYPE_MASK) != PKT_TYPE_ACK) ) {
                ...
                Serial.println(packet_received.type);
                Serial.println(F("** The packet type is incorrect **"));
#endif            	
				return state;	
            }             
		...
		if (_reception == CORRECT_PACKET) {
			...
			else
			{
				...
				{
					 /*   Serial.print("0x");
                        Serial.print(packet_received.data[i] < 16 ? "0" : "");
                        Serial.print(packet_received.data[i], HEX);
                        Serial.print(" ");
                        if (i != _payloadlength - 1) Serial.print(F(","));*/
				}
                Serial.print(F("]"));
				Serial.println();
