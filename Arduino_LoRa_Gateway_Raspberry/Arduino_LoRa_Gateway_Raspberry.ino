...
void decodeData(uint8_t* buffer, int size, int src, int pktnum) {
  ...
  */
  ...
  // i = 0 ; // LPP_DATATYPE_ACCELEROMETER
  // #define LPP_DATATYPE_ACCELEROMETER  0x71
  if(buffer[0] == 113) {
    value = (buffer[1] * 256) + buffer[2];
    PRINT_CSTSTR("%s","\"accelerometer\":{\"x\":");
    PRINT_VALUE("%f", value);
    value = (buffer[3] * 256) + buffer[4]; 
    PRINT_CSTSTR("%s",",\"y\":");
    PRINT_VALUE("%f", value);
    value = (buffer[5] * 256) + buffer[6];
    PRINT_CSTSTR("%s",",\"z\":");
    PRINT_VALUE("%f", value);
    PRINT_CSTSTR("%s","},");
    
  } else {
    Serial.print("\"accelerometer\":{\"x\":0,\"y\":0,\"z\":0},");
  }
  
  
  // i = 7 ; // LPP_DATATYPE_GPSLOCATION
  // #define LPP_DATATYPE_GPSLOCATION    0x88
  if(buffer[7] == 136) {
    
    PRINT_CSTSTR("%s","\"latitude\":");
    
    uint32_t buf1 = buffer[8];
    uint32_t buf2 = buffer[9];
    uint32_t buf3 = buffer[10];
    uint32_t buf4 = buffer[11];
    ...
    Serial.print("\"latitude\":0,\"longitude\":0,");
  }
  ...
  */
  ...
  // i = 16 ; // LPP_DATATYPE_ANALOG_INPUT
