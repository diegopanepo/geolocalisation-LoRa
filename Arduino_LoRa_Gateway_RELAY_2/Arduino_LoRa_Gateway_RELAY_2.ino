...
uint8_t loraChannelIndex=13;   // For 868.0   JFL
...
void decodeData(uint8_t* buffer, int size, int src, int pktnum) {

int i = 0;
float value = 0; // modify before int
double valuel = 0; // modify before long

//PRINTLN;
PRINT_CSTSTR("%s","{\"source\":");
PRINT_VALUE("%d", src); 
PRINT_CSTSTR("%s",",");

PRINT_CSTSTR("%s","\"packet_number\":");
PRINT_VALUE("%d", pktnum); 
PRINT_CSTSTR("%s",",");

i = 1 ; // LPP_DATATYPE_BAROMETER
value = (buffer[2]  * 256) + buffer[3];
/*PRINT_CSTSTR("%s","\"pressure\":");
PRINT_VALUE("%f", value/10.);  
PRINT_CSTSTR("%s",",");
   
i = 5 ; // LPP_DATATYPE_TEMPERATURE
value = (buffer[6]  * 256) + buffer[7];
PRINT_CSTSTR("%s","\"temperature\":");
PRINT_VALUE("%f", value/10.); //modif
PRINT_CSTSTR("%s",",");

i = 9 ; // LPP_DATATYPE_HUMIDITY
value = buffer[10];
PRINT_CSTSTR("%s","\"humidity\":");
PRINT_VALUE("%f", value/2.); //modif
PRINT_CSTSTR("%s",",");


i = 12 ; // LPP_DATATYPE_ACCELEROMETER
value = (buffer[13] * 256) + buffer[14];
PRINT_CSTSTR("%s","\"accelerometer\":{\"x\":");
PRINT_VALUE("%f", value);
value = (buffer[15] * 256) + buffer[16]; 
PRINT_CSTSTR("%s",",\"y\":");
PRINT_VALUE("%f", value);
value = (buffer[17] * 256) + buffer[18];
PRINT_CSTSTR("%s",",\"z\":");
PRINT_VALUE("%f", value);
PRINT_CSTSTR("%s","},");
PRINTLN;
*/
i = 20 ; // LPP_DATATYPE_GPSLOCATION
//valuel = (buffer[21]* 16777216) + (buffer[22] * 65536) +  buffer[23]* 256 + buffer[24];
//valuel = (* 16777216) + (buffer[22] * 65536) +  buffer[23]* 256 + buffer[24];
PRINT_CSTSTR("%s","\"latitude\":");
//PRINTLN;
uint32_t buf1=buffer[21];
//Serial.print("buf21=");
//Serial.println(buffer[21]);
uint32_t buf2=buffer[22];
//Serial.print("buf22=");
//Serial.println(buffer[22]);
uint32_t buf3=buffer[23];
//Serial.print("buf23=");
//Serial.println(buffer[23]);
uint32_t buf4=buffer[24];
//Serial.print("buf24=");
//Serial.println(buffer[24]);
unsigned long int a =buf1*16777216;
unsigned long int b=buf2* 65536;

unsigned long int c=buf3* 256;
unsigned long int d=buf4;
unsigned long int e=a+b+c+d;

//Serial.print(6*65536+168*256+16);
/*Serial.print("a=");
Serial.println(a);
Serial.print("b=");
Serial.println(b);
...
//valuel = 3; // modify
//valuel = (4 * 16777216) + (51 * 65536) +  244* 256 + 96 ;
PRINT_CSTSTR("%s","\"longitude\":");
//PRINTLN;
 buf1=buffer[25];
 //Serial.print("buf25=");
//Serial.println(buffer[25]);
 buf2=buffer[26];
 //Serial.print("buf26=");
//Serial.println(buffer[26]);
 buf3=buffer[27];
//Serial.print("buf27=");
//Serial.println(buffer[27]);
 buf4=buffer[28];
 //Serial.print("buf28=");
//Serial.println(buffer[28]);
 a=buf1*16777216;
 b=buf2* 65536;
 c=buf3* 256;
 d=buf4;
...
z =e/10000000.;
Serial.print(z,7);
//PRINT_VALUE("%d", valuel/10000000);
//Serial.print( valuel/10000,7); 
PRINT_CSTSTR("%s",",");
/*
value = (buffer[27] * 65536) + (buffer[28] * 256) +  buffer[29] ;
PRINT_CSTSTR("%l","\"altitudeGps\":");
PRINT_VALUE("%d", value/100); 
PRINT_CSTSTR("%s",",");
*/
PRINT_CSTSTR("%s","\"RSSI\":");
//Serial.print(valuel/10000., 4);
PRINT_VALUE("%d", sx1272._RSSIpacket); 
PRINT_CSTSTR("%s",",");

i = 31 ; // LPP_DATATYPE_ANALOG_INPUT
value = (buffer[34] * 256) + buffer[35];
PRINT_CSTSTR("%s","\"battery_voltage\":");
PRINT_VALUE("%f", value/100); //modify
PRINT_CSTSTR("%s","}");
//PRINTLN;
/*
i = 38 ; // LPP_DATATYPE_GPSTIME
valuel = 0;
valuel = buffer[39] << 24 + buffer[40] << 16 + buffer[41] << 8 +  buffer[42] ;
PRINT_CSTSTR("%s","\"gps_epoch_value\":");
PRINT_VALUE("%d", valuel); 
PRINT_CSTSTR("%s","}");
*/
}   
...
void startConfig() {
  ...
  if (loraMode==11) {
    ...
    if (optFQ<0.0) {
    	...
    	PRINT_CSTSTR("%s","^$Set frequency to 868.0MHz: state ");
    	e = sx1272.setChannel(CH_17_868); //JFL
    	optFQ=868.0;
    	PRINT_CSTSTR("%s","^$Set frequency to 868.0MHz: state ");
...
//JFL #if defined ARDUINO && not defined GW_RELAY
#if defined ARDUINO 
...
//
  if (radioON && !receivedFromSerial) {
      ...
      //PRINT_CSTSTR("%s","Calling receivePacketTimeout..."); PRINTLN;      //JFL
      e = sx1272.receivePacketTimeout(MAX_TIMEOUT);
      //PRINT_CSTSTR("%s","return from receivePacketTimeout... value:");   //JFL
      //PRINT_VALUE("%d", e);PRINTLN;  //JFL
...
// provide reception timestamp
         ...
         //PRINT_STR("%s", "TMP_LENGTH: ");
         //PRINT_VALUE("%d",tmp_length);
         PRINTLN;
...
//         PRINT_STR("%c",(char)DATA_PREFIX_1);
         ...
         //sx1272._nodeAddress=6;
         sx1272._packetNumber=sx1272.packet_received.packnum;
         sx1272.setPacketType(sx1272.packet_received.type);

         CarrierSense();
         // check ID_Relay
         if( sx1272.packet_received.data[19]== 0x00 || sx1272.packet_received.data[19]== 0x01){
           sx1272.packet_received.data[19] += 0x10; //JP
           Serial.print("buffer[16]==2 ");
           for(int i=0; i<=19;i++){
           Serial.println(sx1272.packet_received.data[i]);
           }
           
           Serial.print("buffer relai: ");
           Serial.println(sx1272.packet_received.data[19]);
           e = sx1272.sendPacketTimeout(sx1272._nodeAddress, sx1272.packet_received.data, tmp_length, 10000);
       ...
       //  Serial.print(F("Data: ["));
         ...
         {
             /* Serial.print("0x");
              Serial.print(sx1272.packet_received.data[i] < 16 ? "0" : "");
              Serial.print(sx1272.packet_received.data[i], HEX);
             Serial.print(" ");
              if (i != tmp_length - 1) Serial.print(F(","));
               */
        }
             // Serial.print(F("]"));
              
        decodeData(sx1272.packet_received.data,tmp_length, sx1272.packet_received.src, sx1272.packet_received.packnum );
/* JFL         
         for ( ; a<tmp_length; a++) {
